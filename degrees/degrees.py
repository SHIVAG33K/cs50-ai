...
def shortest_path(source, target):
    ...
    while True:
        ...
        for action , new_state in neighbors_for_person(node.state):
            if not Queue_Frontier.contains_state(new_state) and new_state not in explored_states:
                ...
                if child_node.state == target:
                    ...
                    while child_node.parent is not None:
                        actions.append(child_node.action)
                        states.append(child_node.state)
                        child_node = child_node.parent
                    actions.reverse()
                    states.reverse()
                    elements = zip(actions,states)
                    for i , j in elements:
                        result.append((i,j))
                    return result
